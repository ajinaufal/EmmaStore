...
// 22. Counter JS
// 23. Tooltip JS
// 24. Update price according to varian type
// 26. Cookiebar
// 27. Quick View
// 28. Get City
  ...
  $(".tabs li a").on('click', function () {
    ...
    $('.media-slide-5').slick({
      ...
      $('.quick-view').on('click', function () {
        $('.quick-view-img').css('background-image', 'url("")');
		$('.quick-view-img .img-fluid').attr("src", "");
		$('.pro-group h2').html("");
		$('.pro-price').html('');
		$('#qv-product-info p').html("");
		$('#qv-varian').html("");
        ...
        $.ajax({
			...
			success: function(response) {
			    ...
			    var product = resp.product[0];
			    $('.quick-view-img').css('background-image', 'url("/assets/images/products/parfum/' + product.gambar1 + '")');
				$('.quick-view-img .img-fluid').attr("src", "/assets/images/products/parfum/" + product.gambar1);
				$('.pro-group h2').html(product.nama);
				if(product.onsale > 0)
				{
				    var discpercent = parseFloat(((product.harga)-(product.onsale))/(product.harga)*100);
				    $('.pro-price').html('<li>Rp. '+addCommas(product.onsale)+'.00</li><li><span>Rp. '+addCommas(product.harga)+'.00</span></li><li>'+discpercent.toFixed(2)+'% off</li>');
				}
				else {
				    $('.pro-price').html('<li>Rp. '+addCommas(product.harga)+'.00</li>');
				}
				$('#qv-product-info p').html(product.deskripsi);
				if(!$.isEmptyObject(resp.product_varian)) {
				    var varian = '<h6 class="product-title">Pilih '+resp.product_varian[0].jenis+'<div class="size-box"><ul>';
				    $.each( resp.product_varian, function( idx, val ) {
					    varian = varian + '<li ><a href="javascript:void(0)">'+val.nama+'</a></li>';
					});
					varian = varian + '</ul></div>';
				    $('#qv-varian').html(varian);
				}
				$('.size-box ul li').on('click', function(e) {   
...
$('.tap-top').on('click', function() {
  ...
  $('.quantity-left-minus').on('click',function(){
    ...
    $('.prod_var').on('click', function() {
        var harga = this.dataset.harga;
        var onsale = this.dataset.onsale;
        var price = "";
        if(onsale > 0) {
            price = '<li>Rp. '+addCommas(onsale)+'.00</li><li><span>Rp.'+addCommas(harga)+'.00</span></li><li>'+((harga-onsale)/harga*100)+'% off</li>';
        }
        else {
            price = '<li>Rp. '+addCommas(harga)+'.00</li>';
        }
